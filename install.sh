#!/usr/bin/env bash
# Repo Tour installer (macOS / Linux)
#
# Usage:
...
#
# Environment:
#   RT_REPO_URL  Override clone URL (default: official GitHub repo)
#   RT_DIR       Override clone destination (default: $HOME/.repo-tour/repo)

set -euo pipefail

REPO_URL="${RT_REPO_URL:-https://github.com/HappyOnigiri/RepoTour.git}"
REPO_DIR="${RT_DIR:-$HOME/.repo-tour/repo}"
...
link_skills() {
  ...
  case "$style" in
    per-skill)
      ...
      ;;
    folder)
      ln -sfn "$root" "$target/repo-tour"
      printf '  ✓ %s → %s\n' "$target/repo-tour" "$root"
      ;;
    *)
    ...
    per-skill)
      if [[ -d "$(skills_root)" ]]; then
        ...
        for link in "$target"/*; do
          ...
          [[ "$resolved" == *"/RepoTour/"*"/skills/"* ]] || continue
          rm -f "$link"
        done
      fi
      ;;
    folder)
      [[ -L "$target/repo-tour" ]] && rm -f "$target/repo-tour"
      ;;
  esac
}
...
Environment:
  RT_REPO_URL  Override clone URL (default: official repo)
  RT_DIR       Override clone destination (default: \$HOME/.repo-tour/repo)
USAGE
}
